...
void* handleConnection(void* arg){
    ...
    ServerConfig* server_config = thread_struct->server_config; 
    int v_flag = server_config->verbose_mode; 
    ...
    while(1){
        ...
        if (write(client_fd, buffer, number_of_bytes_read) < 0) { // Echo the data back to the client and check for write error
            perror("Error writing to client");
            break;
        }

        if (v_flag) {
            printf("Echoed %zd bytes from Connection on %d: %.*s", number_of_bytes_read, client_fd, (int)number_of_bytes_read, buffer);
        }
    }
